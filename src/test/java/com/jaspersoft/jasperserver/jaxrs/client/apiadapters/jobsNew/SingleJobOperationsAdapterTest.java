...
public class SingleJobOperationsAdapterTest extends PowerMockTestCase {
    ...
    @Mock
    private JerseyRequest<ClientJobState> jobStateJerseyRequestMock;

    @Mock
    private OperationResult<ClientJobState> jobStateOperationResultMock;

    @Mock
    private OperationResult<ClientReportJob> jobOperationResultMock;
    ...
    public void should_return_proper_job_operation_result_when_JRS_version_is_v5_6_1() {
        ...
        when(JerseyRequest.buildRequest(eq(sessionStorageMock), eq(ClientReportJob.class), eq(new String[]{"jobs", expectedJobId}))).thenReturn(jobRequestMock);
    ...
    public void should_proper_op_result_instance() {
        ...
        // Given
        mockStatic(JerseyRequest.class);
        when(JerseyRequest.buildRequest(eq(sessionStorageMock), eq(ClientJobState.class),
    ...
    public void should_return_prepared_op_result_when_jrs_version_is_5_6_1() {
        ...
        // Given
        mockStatic(JerseyRequest.class);
        when(JerseyRequest.buildRequest(eq(sessionStorageMock), eq(ClientReportJob.class), eq(new String[]{"jobs", expectedJobId}), any(JobValidationErrorHandler.class))).thenReturn(jobRequestMock);
    ...
    public void should_return_RequestExecution_when_environment_is_specified() throws Exception {
        ...
        Callback<OperationResult<ClientReportJob>, Void> callback = PowerMockito.spy(new Callback<OperationResult<ClientReportJob>, Void>() {
    ...
    public void should_retrieve_state_asynchronously() throws InterruptedException {
        ...
        PowerMockito.when(JerseyRequest.buildRequest(
                eq(sessionStorageMock),
                eq(ClientJobState.class),
                eq(new String[]{"jobs", "123435326", "state"})))
                .thenReturn(jobStateJerseyRequestMock);

        SingleJobOperationsAdapter adapterSpy = PowerMockito.spy(new SingleJobOperationsAdapter(sessionStorageMock, "123435326"));
        Callback<OperationResult<ClientJobState>, Void> callback = PowerMockito.spy(new Callback<OperationResult<ClientJobState>, Void>() {
            public Void execute(OperationResult<ClientJobState> data) {
                newThreadId.set((int) Thread.currentThread().getId());
                synchronized (this) {
                    this.notify();
                }
